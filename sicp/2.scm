...
(define (encode-symbol symbol tree)
  (cond ((null? tree) (error "Does not exists the symbol"))
        ((eq? symbol (symbol-leaf (left-branch tree))) 
         (list 0))
        ((and (leaf? (right-branch tree))
              (eq? symbol (symbol-leaf (right-branch tree))))
          (list 1))
        (else (append (list 1) (encode-symbol symbol (right-branch tree))))))


(define sample-message-chars (decode sample-message sample-tree))

(define chars '(a d a b b c a))
(define chars '(c))

(encode-symbol 'a sample-tree)
(encode chars sample-tree)
sample-message

; Complex Numbers (copiado del libro)
...
(define (make-from-mag-ang-rectangular z r a)
...
(define table '())
(define (put key type proc)
  (append table (list key (list type proc))))

(define table (put 'real-part 'rectangular (lambda (z) (car z))))
(define table (put 'imag-part 'rectangular (lambda (z) (cdr z))))
